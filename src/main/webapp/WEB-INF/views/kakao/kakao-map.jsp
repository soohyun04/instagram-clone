...
<div class="map-wrap">
    ...
    <div class="side-panel">
        ...
        items      반복할 데이터 목록
        var        반복할 때마다 하나씩 꺼낸 값을 담는 변수명
        varStatus  반복 상태 정보를 담는 변수
                   varStatus = "status"
                                status.index -> 현재 순서 0번째부터 순번 부여받음
                                status.count -> 현재 순서 1번째부터 순번 부여 총 개수 length 형태
                                status.first -> 첫번째 항목이면 true
...
<script>
    ...
    const locations = [
        ...
        <%--현재 loc 에서 받은 locations 데이터가 맨 마지막 데이터가 아닌게 맞다면 맨뒤에, 를 추가하라--%>
    ...
    //=======================================================
    // placeList
    // 위에서 백엔드로 전달받은 데이터의 형태를 html로 직접적으로 작성했기 때문에innerHtml에서 추가해줄 필요가 없다.
...
<%--    // forEach 문은 목록들을 0번째부터 끝번째까지 순회--%>
<%--    locations.forEach((loc, idx) => {--%>
<%--        console.log("현재 log : " , loc); //locations 을 순회해서 loc안에 1번 부터 5번까지 모두 다 조회가 되는지 확인--%>
...
<%--*!/!*!/!*!/*/--%>
    ...
    function 지도초기화() {
        ...
        locations.forEach((loc, idx) => {
            ...
            //카카오 맵에서 내부적으로 HTML을 이용해서 받는 형태로 코드자체가 되어있기 때문에
            //html 작성 불가
            const 인포윈도우내용 = `
...
<%--
카카오에서 플랫폼 키를 가져올 때 , 테스트앱 -> 비즈앱에 되어 있는 키를 가져온다.
비즈앱에 있는 키의 경ㅇ우 사업자등록을 한 사람들이 정식적으로 운영하는 서비스이고,
이 서비스를 운영하기 위해서 개발자가 테스트하는 용도로 총 5개까지 만들어서 테스트 할 수 있도록 테스트앱 제공
